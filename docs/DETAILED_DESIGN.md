...
✅ **main 自動反映**: 生成された内容を `main` へ自動コミット  
✅ **Web サイト化**: Astro で Web サイトとして自動デプロイ可能
...
スケジュール実行時は日本時間の日付を対象日として渡し、`cache/blogs` の日次スナップショットを `main` へコミットする。これにより翌日の差分検出で前日キャッシュを利用できる。
