...
<body>
  ...
  <footer>Generated by <strong>${escapeHtml(SITE_TITLE)}</strong> &mdash; Powered by GitHub Actions &amp; Agentic Workflows</footer>
</body>
</html>`;
}
...
function buildIndexPage(dates) {
  ...
  const cards = dates
    ...
    .map(
      (date) =>
        `<div class="card"><a href="./${date}.html">${escapeHtml(date)}</a><div class="meta">Daily update</div></div>`,
    )
    .join('\n');

  const body = `
    <h1>更新履歴</h1>
    <p>全 ${dates.length} 日分の Microsoft &amp; GitHub ブログ更新情報</p>
    <div class="card-grid">
      ${cards}
    </div>`;
...
async function main() {
  ...
  for (const file of mdFiles) {
    ...
    const html = pageShell({
      ...
      description: `${date} の Microsoft & GitHub ブログ更新情報`,
      body,
      backLink: true,
    });
